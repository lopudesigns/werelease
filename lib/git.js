...
const tag = (version, tag, annotation) => {
  ...
  return run(`git tag ${force}--annotate --message="${message}" ${formattedVersion}`).catch(err => {
    debugGit(err);
    warn(`Could not tag. Does tag "${version}" already exist? Use --force to move a tag.`);
  });
};
...
const push = async ({ pushRepo = '', hasUpstreamBranch, args = '', dontSetUpstream, dontUseUrl } = {}) => {
  const setUpstream = hasUpstreamBranch === false ? dontSetUpstream ? `origin ${await getBranchName()}` : `--set-upstream origin ${await getBranchName()}` : '';
  return run(`git push --follow-tags ${args} ${dontUseUrl ? '' : pushRepo} ${setUpstream}`);
};
